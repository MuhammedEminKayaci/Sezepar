    ...
    <>
      <header
        className={`sticky top-0 z-50 transition-all duration-500 ${
          scrolled
            ? "bg-white/95 backdrop-blur-2xl shadow-[0_1px_3px_rgba(0,0,0,0.08)] border-b border-[#e5e7eb]"
      ...
      >
          ...
          <div className="flex items-center h-16 sm:h-[68px]">
            ...
            <Link href="/" className="shrink-0 flex items-center gap-2.5 group mr-8">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] text-[#9ca3af] font-medium tracking-[0.2em] uppercase leading-none mt-0.5">
                  Yedek Parça
                </span>
              </div>
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-0.5 flex-1 justify-center">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="relative px-3 xl:px-3.5 py-2 text-[13px] font-medium text-[#6b7280] hover:text-[#111827] rounded-lg hover:bg-[#f3f4f6] transition-all duration-200 group whitespace-nowrap"
                >
                  {link.name}
            ...
            <div className="flex items-center gap-1 ml-auto md:ml-0">
              ...
              <button
                ...
                className="flex items-center gap-2 h-9 px-3 rounded-xl text-[#9ca3af] hover:text-[#111827] hover:bg-[#f3f4f6] transition-all duration-200 group"
                aria-label="Arama"
              >
              ...
              <a
                ...
                className="hidden xl:flex items-center gap-1.5 h-9 px-3 rounded-xl text-[#6b7280] hover:text-[#111827] hover:bg-[#f3f4f6] transition-all duration-200"
              >
        ...
        >
          ...
          <div className={`absolute top-0 right-0 w-[300px] max-w-[85vw] h-full bg-white border-l border-[#e5e7eb] shadow-2xl transition-transform duration-300 ease-out flex flex-col ${mobileOpen ? "translate-x-0" : "translate-x-full"}`}>
            {/* Close button */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e5e7eb]">
              <span className="text-sm font-bold text-[#111827]">Menü</span>
              <button
                onClick={() => setMobileOpen(false)}
                className="w-8 h-8 rounded-lg flex items-center justify-center text-[#9ca3af] hover:text-[#111827] hover:bg-[#f3f4f6] transition-colors"
                aria-label="Kapat"
              >
            ...
            <nav className="flex-1 overflow-y-auto py-3 px-3">
              {NAV_LINKS.map((link, i) => (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setMobileOpen(false)}
                  className={`flex items-center px-4 py-3.5 text-[15px] font-medium text-[#374151] hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all duration-200 ${mobileOpen ? "translate-x-0 opacity-100" : "translate-x-4 opacity-0"}`}
              ...
              {/* Divider */}
              <div className="mx-4 my-3 border-t border-[#e5e7eb]" />

              {/* Phone link */}
              <a
                href={`tel:${SITE_CONFIG.phone}`}
                className="flex items-center gap-3 px-4 py-3.5 text-[15px] font-medium text-[#374151] hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all duration-200"
              >
            ...
            <div className="p-4 space-y-2 border-t border-[#e5e7eb]">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#20bd5a] text-white text-sm font-bold px-4 py-3.5 rounded-xl transition-colors"
              >
              ...
              <Link
                href="/iletisim"
                onClick={() => setMobileOpen(false)}
                className="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-amber-500 to-amber-600 text-black text-sm font-bold px-4 py-3.5 rounded-xl transition-colors"
              >
                Teklif Al
              </Link>
            </div>
